...
공개 사이트는 공개용 MSDS 데이터와 `pdf/` 폴더의 대표 PDF 224개를 사용합니다. 로컬 실행은 `start_msds_site.bat`로 실행하며, local JSON이 있으면 local 데이터를 우선 사용합니다.
...
- 공개 운영 안내 문구 표시 정상
- 사이트 내부 전체 페이지 PDF 미리보기 정상
- `pdf/` 폴더의 대표 PDF 224개 공개 접근 가능
...
PDF 영역에는 먼저 파일 경로와 `PDF 미리보기` 버튼만 표시합니다. 사용자가 버튼을 누른 경우에만 PDF.js로 사이트 안에 PDF 전체 페이지 미리보기를 표시합니다. 뷰어 안에서는 전체 페이지를 세로 스크롤로 확인할 수 있고, 확대, 축소, 폭맞춤을 사용할 수 있습니다.

PDF는 GitHub Pages에 공개되어 있으므로 PDF 주소를 직접 알면 기술적으로 다운로드를 완전히 막을 수는 없습니다. 다만 사이트 UI에서는 다운로드나 새 탭 열기를 제공하지 않고, 사용자가 사이트 안에서 전체 PDF를 미리보기로 확인하도록 구성합니다.
