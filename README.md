...
새 판에서 신호어·그림문자·유해위험문구·예방조치문구가 달라졌으면 그 이력 줄에 `labelReprint: true` 와 까닭을 적습니다.
경고표지 화면(반년 동안)과 관리대장 화면의 "표지를 다시 뽑을 제품"에 뜹니다.

### 문구 꼴과 원본 PDF 대조 보완

`scripts/build_public_data.py` 는 관리대장 반영 뒤에 `scripts/normalize_public_content.py` 를 부릅니다. 몇 번을 돌려도 결과가 같습니다.

- **문구 꼴**: 유해·위험문구와 예방조치문구 앞의 `-`, `예방`, `유해·위험문구`, `P201 :` 같은 것을 떼고 "코드 문구" 꼴로 맞춥니다.
  앞 문구 꼬리("받으시오.")가 다음 줄에 붙어 있으면 앞 문구로 돌려줍니다. 문구 글자와 코드는 바꾸지 않습니다.
- **칸 나누기**: 예방조치문구를 코드 번호로 나눕니다(P1·P2 예방, P3 대응, P4 저장, P5 폐기). 대응 문구가 예방 칸에 있으면
  관리요령의 누출·화재 시 칸이 비기 때문입니다(2026-09-23 기준 86건이 그랬음).
- **PDF 대조 보완**: `data/msds-content-repairs.json`. 응급조치의 빈 칸, 깨진 성분 이름, 사이트가 빠뜨린 유해·위험문구를
  원문에서 읽어 채웁니다. 지금 값이 적어 둔 "고치기 전" 값일 때만 바꿉니다.
- **hazardBadge**: 공개하지 않습니다. 신호어는 `signalWord` 하나입니다.
- **띄어쓰기**: 조사 앞 빈칸("스프레이 의"), 동작 명사와 "하다" 사이 빈칸("실시 하시오")을 뗍니다. 한 글자씩 벌어졌거나
  붙어 버린 문구는 다른 제품의 같은 코드·같은 글자 문구의 띄어쓰기를 빌립니다. 글자는 바꾸지 않습니다.
- **그림문자**: PDF 안의 그림이라 글자로 읽을 수 없어, 원문 제2항을 눈으로 보고 확인한 것만 `ghsCodes` 보완에 적습니다
  (`verifiedBy` 에 무엇을 봤는지). 원문 스스로 앞뒤가 안 맞으면(분류와 경고표지 항목이 다름) 고치지 않고 관리대장
  `sourceIssue`/`sourceIssueAsk` 에 적어 공급사에 묻습니다.

`scripts/repair_from_pdf.py` 는 두 칸 표 PDF 처럼 pypdf 가 글 순서를 뒤섞는 경우 PyMuPDF(`pymupdf`)로 한 번 더 읽습니다.
설치돼 있지 않으면 그 단계만 건너뜁니다. 글꼴이 깨진 PDF("싞발을 벖고")의 글은 넣지 않습니다.

관리대장 화면의 "공급사별 할 일"에는 공급사마다 **요청 문안 복사** 단추가 있습니다(보내지는 않음). 라벨 확인처럼 우리 쪽에서
할 일은 문안에 넣지 않습니다. 추가 확인 제품 체크리스트는 원본 보관함 맨 위 `MSDS_추가확인_체크리스트.csv` 에 있습니다.

원본 PDF 를 다시 대조하려면(읽기만 하려면 옵션 없이):

```
py scripts/repair_from_pdf.py --write --write-register
```

`scripts/msds_pdf_text.py` 가 모든 PDF 에 같은 규칙을 씁니다. 절 제목을 번호+제목으로 찾아(글자층에서 절 순서가 뒤바뀐 PDF 도 있음)
그 절만 읽고, 코드가 문장 앞에 오든 뒤에 오든 읽습니다. 날짜는 개정 이력이 줄줄이 적힌 꼴("16차/2019.01.16, 17차/…")이면
가장 늦은 날을 최종 개정일로 봅니다. 관리대장에 날짜를 적을 때는 그 PDF 의 SHA-256(`datesCheckedSha256`)을 같이 적어,
같은 자리에 새 PDF 가 들어오면 옛 날짜로 덮지 않습니다.

### 올리기 전 점검

데이터나 PDF 를 바꿨으면 커밋 전에 한 줄로 돌립니다. 관리대장 반영, 문구 꼴·보완 반영, 판 번호, 테스트,
배포 목록(`data/release-manifest.json`) 다시 만들기, CI 와 같은 검증까지 차례로 하고 실패하면 멈춥니다.

```
py scripts/prepare_release.py
```

배포 목록을 다시 만들지 않으면 CI 가 `RELEASE_MANIFEST_STALE` 로 실패합니다(2026-09-18~23 에 실제로 닷새 동안 그랬음).
